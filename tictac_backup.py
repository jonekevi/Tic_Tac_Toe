...
class main:
    ...
    def max_move(self, state):
        ...
        for m in moves:
            r, c = m
            state= [row[:] for row in temp]
            temp[r][c] = self.cur_player
            if(debug):
                print ("Marking row " , str(r),  " and column " , str(c) + " for player " + str(self.cur_player) + "\n")
                print (state)
            score, move = self.min_move(state)
            if (score > best_score):
                best_score = score
                best_move = m
        print("Max move -- Best score = " + str(best_score) + " Best move = " + str(best_move) + "\n")
    ...
    def min_move(self, state):
        ...
        for m in moves:
            r, c = m
            state= [row[:] for row in temp]
            state[r][c] = (self.cur_player+1)%2
            if(debug):
                print ("Marking row " + str(r) + " and column " + str(c) + " for player " + str((self.cur_player+1)%2) + "\n")
                print (state)
            score, move = self.max_move(state)
            if (score < best_score):
                best_score = score
                best_move = m
        print("Min Move -- Best score = " + str(best_score) + " Best move = " + str(best_move) + "\n")
        return best_score, best_move
