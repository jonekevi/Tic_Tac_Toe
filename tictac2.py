...
class main:
    ...
    def	move_minimax(self):
        ...
        branch = copy.deepcopy(self.TTT)
        v, move = self.max_move(branch)
    ...
    def max_move(self, state):
        ...
        if(self.check_term()!= -1):
            score = self.check_term()
            return score, None

        branch = state
        for m in moves:
            r, c = m
            branch[r][c] = self.cur_player
            print ("Marking row " , str(r),  " and column " , str(c) + " for player " + str(self.cur_player) + "\n")
            print (branch)
            score, move = self.min_move(branch)
            if (best_score < score):
                best_score = score
                best_move = m
